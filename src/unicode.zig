// Source was taken from https://github.com/ziglang/zig/blob/971ef7b9c2d67ee849e252e75b79ee1e1ef3da6f/lib/std/unicode.zig
// with modifications to base most functions on `Char`
...
pub const Char = struct {
    ...
    pub fn init(codepoint: Codepoint) !Self {
        return switch (codepoint) {
            0xD800...0xDFFF, // Reserved surrogates range
        ...
        switch (length) {
            ...
            3 => {
                switch (self.codepoint) {
                    0xD800...0xDFFF => unreachable, // Surrogates range
                    else => {},
                }
...
fn testUtf8EncodeError() !void {
    try testing.expectError(error.InvalidCodepoint, Char.init(0xd800));
    try testing.expectError(error.InvalidCodepoint, Char.init(0xdfff));
    try testing.expectError(error.InvalidCodepoint, Char.init(0x110000));
    try testing.expectError(error.InvalidCodepoint, Char.init(0x1fffff));
}
...
pub fn utf16leToUtf8(utf8: []u8, utf16le: []const u16) !usize {
    ...
    var it = Utf16LeIterator.init(utf16le);
    while (try it.nextChar()) |char| {
        end_index += try char.utf8Encode(utf8[end_index..]);
    }
    return end_index;
}
...
fn formatUtf16le(
    ...
    const replacement_character = Char{ .codepoint = 0xfffd };
    _ = fmt;
    _ = options;
